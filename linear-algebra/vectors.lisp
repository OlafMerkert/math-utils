...
(defmacro make-vector (dimensions index-vars &body fill-form)
  "Convenience macro for building vectors, indexing starts with 0. The
array to be filled can be referenced by this in fill form, but do not
rely on filling order."
  (unless index-vars
    (setf index-vars (mapcar (lambda (x) (symb 'i x)) (lrange dimensions))))
...
(defmacro make-vector/human (dimensions index-vars &body fill-form)
  "convenience macro for building vectors, indexing starts with
1 (both for autogenerated index-vars and actual indices)."
  (unless index-vars
    (setf index-vars (mapcar (lambda (x) (symb 'i (+ x 1))) (lrange dimensions))))
  (unless (or (member '&rest index-vars)
              (= (length dimensions) (length index-vars)))
    (error "Mismatching dimensions and indices ~A!" index-vars))
  `(make-vector% (list ,@dimensions)
                 (lambda (this ,@index-vars)
                   ...
                   ;; increase all the indices
                   ,@(mapcar #`(incf ,a1) index-vars)
                   ,@fill-form)))
